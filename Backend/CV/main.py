...
def parse_args(argv: Optional[Sequence[str]] = None) -> TrainConfig:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("./model"), help="Where to store checkpoints and logs")
...
def main(cfg: Optional[TrainConfig] = None) -> None:
    ...
    with history_path.open("w", encoding="utf-8") as history_file:
        ...
        for epoch in range(1, cfg.epochs + 1):
            ...
            if val_acc > best_acc:
                best_acc = val_acc
                # Save only the model state_dict to avoid pickle issues
